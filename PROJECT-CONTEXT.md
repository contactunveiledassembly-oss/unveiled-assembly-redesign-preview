...
### 27.1 Demo member quick sign-in — v1, then corrected same day
**v1 (superseded — see below):** added a "Try A Demo Member Account" button to the public Sign In screen, calling `signInWithEmailAndPassword(auth, 'demo@example.com', 'preview-demo')`. Committed, pushed (`demo-member-quick-signin`), opened as PR #5, and merged into `main`.
**Correction, same day:** the owner actually wanted this reachable from *inside the Owner Portal* (a quick way for the owner themselves to jump into the member experience — the existing "Member Experience Preview" tickets/classes example — not a public-facing button on the Sign In screen). **Removed** the Sign In button entirely (reverted that markup/script block). **Instead**, discovered the Owner Portal already has a full "Owner Member View" preview system (`enterOwnerMemberPreview()`, `ownerPreviewMember`, the `#ownerHeaderMemberViewBtn` "Member View" button already sitting in the Owner Portal's own header) that lets an admin preview the Member Portal as any real member — but its one existing one-click header button previously called `enterOwnerMemberPreview(null)`, which lands on a **generic, empty** simulated member with no data, not the rich "Maya Johnson" example (a *different* identity, `uid: 'demo-member'`, only reachable before now by typing her name into the in-preview search box). Changed that one call site to `enterOwnerMemberPreview('demo-member')`, so the owner's existing "Member View" header button now lands directly on Maya Johnson's fully populated example (her confirmed one-on-one booking/ticket, class registrations, notifications) with a single click, no typing required — exactly "the constant example... like it was in the owner [portal]" the owner described. In production this same button safely falls back to the prior blank-preview behavior, since `'demo-member'` won't match any real account there (`resolveAllMemberProfiles()` is demo-data-only) — no risk of ever showing the wrong person's data.
...
### 27.4 The real reason the owner still didn't see any of this — a sitewide stale-cache bug, found and fixed
The owner reported still not seeing the Member View / Maya Johnson update after it was merged and the GitHub Pages build had genuinely succeeded (verified directly: the live `app.js` on `https://contactunveiledassembly-oss.github.io/...` already contained `enterOwnerMemberPreview('demo-member')` and the new `demo-warfare` Zoom entry — this was not a deploy failure). The actual cause: **11 of the site's 13 pages still referenced `app.js?v=phase2-correction2-20260921` and `styles.css?v=phase2-correction2-20260921`** — a cache-busting version string last updated in Phase 2, never bumped since, even though `app.js` has been rewritten extensively across Phases 4, 5, and every session since. Only `give.html` and `one-on-one.html` (each individually bumped when it was that page's own turn to change) had a current version string. Anyone whose browser had cached `app.js` under that old URL from an earlier visit — including, very plausibly, the owner's own browser — would keep being served that stale cached copy from `index.html`, `story.html`, `beliefs.html`, `teachings.html`, `prayer.html`, `connect.html`, `gather.html`, `testimonials.html`, `teaching-detail.html`, `404.html`, and `shop/index.html`, with zero visible errors, indefinitely, regardless of how many times `main` was updated — a hard refresh was the only workaround, and nothing on the page would have hinted that was necessary.
**Fixed:** every page's `app.js` and `styles.css` reference now shares one identical, current version string (`sitewide-20260925`), so every page is guaranteed to fetch fresh files after this deploy. **Going forward, this needs discipline this project hasn't had:** any future change to `app.js` or `styles.css` should bump this same version string on every page that loads them (all of them), not just whichever page's HTML happened to be edited that pass — otherwise this exact bug recurs silently.

### 27.5 The file-cache fix wasn't the whole story — a second, deeper localStorage staleness bug, found and fixed
After 27.4 shipped, the owner tried Member View again and got a real, specific report: "Welcome back, Demo Account" (not "Maya Johnson") with an otherwise-empty dashboard — the layout rendering correctly, but no booked classes, no one-on-one, nothing. That specific detail (the generic fallback name, not an error) pointed straight at a second, unrelated bug: **`loadPreviewFromStorage()`** (called unconditionally in `DEMO_MODE`) **unconditionally trusts whatever `DEMO_BOOKINGS`/`DEMO_TEACHING_REGISTRATIONS`/etc. this browser already saved to `localStorage` from any earlier visit, completely overriding the fresh hardcoded seed data in `app.js` with it — no version check, no merge.** The owner's own browser had almost certainly saved a snapshot of that data from an earlier visit, *before* Maya Johnson's booking/registrations/classroom content existed in the code at all — so no matter how many times the seed data was updated and deployed, that browser kept resurrecting its own older, Maya-less snapshot on every load. This is a completely different mechanism from 27.4 (that one was the browser's HTTP cache for *files*; this one is `localStorage`, which HTTP cache-busting has no effect on at all) — fixing one could never have fixed the other, which is exactly why the owner still saw nothing even after 27.4 shipped and was confirmed live.
**Fixed the same way as 27.4, one level deeper:** added `PREVIEW_DATA_VERSION` (currently `'2026-09-25-maya-v2'`) — on `DEMO_MODE` load, if a browser's saved version marker doesn't match, every `PREVIEW_STORAGE_KEYS` entry is cleared before `loadPreviewFromStorage()` runs, so that browser starts fresh from the current hardcoded seed data (which already includes all of Section 27.1–27.3's Maya Johnson content) instead of resurrecting a stale snapshot. **Going forward:** any time the `DEMO_*` seed arrays change in a way a visitor should actually see, `PREVIEW_DATA_VERSION` needs to be bumped too — the same discipline 27.4 already calls for with the file-cache version string, just for a second, separate cache that behaves completely differently and needed its own fix.

---

## 28. Session Log — 2026-09-25: Member Location + Owner Portal Location Analytics (NOT YET COMMITTED — owner review pending)

**New branch: `member-location-analytics`**, created off latest `main` (which already includes everything through Section 27). Nothing in this section has been committed, pushed, merged, or deployed — per explicit instruction. Followed the required pre-work before any edit: re-read this entire file, inspected the real registration form/handler, the real `users/{uid}` Firestore write, `firestore.rules`, and the Owner Portal's People panel directly in the current source (not from memory) before writing anything.

### 28.1 What was actually built
- **Registration (`registerForm` in `app.js`):** a new "Where Are You Located?" block between Phone and Password, using the owner's exact supporting copy. **State** is a required `<select>` of all 50 states + DC (`US_STATES`/`US_STATE_OPTIONS`) — a fixed dropdown, not free text, which is what actually guarantees "a standardized state value" (no fuzzy-matching needed later, since "GA"/"ga"/"Georgia " typed three ways is structurally impossible). **City** is an optional free-text field. No street address, unit, or GPS field exists anywhere.
- **`users/{uid}` schema:** two new fields, `state` (one of the 50 state names, or `null`) and `city` (free text as typed, or `null`). Added to the real registration `setDoc()` call.
- **Member Profile card** (the photo/name/nickname/birthday card built in Section 25): now also has a "Location" block with the same State/City fields and the owner's exact supporting copy ("This information helps us understand..."), editable any time, saved via the same `updateDoc()` the rest of that card already uses.
- **Owner Portal → People → Location Analytics:** a new tab inside the existing People panel (not a new sidebar item — the instruction said "under People," and the existing sidebar nav was explicitly not to be touched). Built:
  - **Overview:** Total Members, States Represented, Cities Represented, Members With Location Data — all four computed live from real member records every time the tab renders, never hardcoded.
  - **Members By State:** a real, computed "Top States" table, plus a visual — see 28.2 for exactly what kind of "map" this is and why.
  - **Top Cities:** computed and normalized — see 28.3.
  - **Regional Analytics:** Southeast/Northeast/Midwest/Southwest/West, count + percentage, using a standard U.S. Census-style regional grouping (`STATE_REGIONS`) — there's no single official version of this breakdown, so this is the conventional one, not an owner decision that was skipped.
  - **Event Insights:** the top 3 city concentrations, labeled "Largest/Second-Largest/Third-Largest Community Concentration" — presents the numbers only. **No recommendation logic of any kind was written** — there's no code anywhere that suggests where to hold an event; the instruction was explicit about this and it was followed exactly.
  - **Filters:** All Time / This Year / This Month, using each member's real `createdAt` — see 28.4 for the one honest limitation this has.

### 28.2 "Members By State" — what the visual actually is, and why
A real, literal U.S. map (accurate state boundary/path geometry) isn't something that can be hand-authored reliably in this environment — there's no mapping library, no geodata file, and no way to visually verify a hand-drawn path is actually correct without a browser. Rather than ship something that might silently misrepresent state shapes/borders, **`.loc-state-grid` is a stylized "tile grid" map** — a common, legitimate alternative for exactly this use case (each state gets one equally-sized tile in roughly its real relative position; every tile always renders, even at zero members, so the overall U.S. shape stays recognizable; only the tile's color intensity changes with member count). This is explicitly labeled as approximate in the code's own comments. The **Top States table next to it is the real, exact, numerically accurate data** — per the instruction's own "don't depend only on the map" requirement, the table (not the tile grid) is the source of truth anywhere the two could be read differently.

### 28.3 City normalization — exactly what changes and what doesn't
Per instruction: a member's own typed spelling is **never altered** — `member.city` (what's stored, what shows on their own profile, what an admin sees on their People record) stays exactly as they typed it. Normalization (`normalizeCityKey()`: lowercase, trimmed, whitespace-collapsed, scoped per-state so "Springfield, IL" and "Springfield, OH" don't merge) is used **only** as the internal grouping key when computing the Top Cities analytics — the label actually displayed in that table (`cityDisplayLabel()`) uses whichever member's real typed spelling was seen first for that normalized key, not a synthesized/title-cased version.

### 28.4 Filters — the one honest limitation, flagged rather than silently handled
`createdAt` already exists on real registrations (added in an earlier phase, confirmed by reading the current registration `setDoc()` before touching it) — so "This Year"/"This Month" are real, not fabricated. **The limitation:** any account created before `createdAt` existed in the schema has no value to filter by at all. Rather than guess (e.g., defaulting them into "this year," which could be wrong) or silently drop them from the total, **a member with no `createdAt` counts fully under "All Time" but is excluded from "This Year"/"This Month"** specifically — an honest gap, not a misleading filter. This is a judgment call made instead of stopping to ask first, on the reasoning that it's the *accurate* option the instruction asked for ("if the data structure doesn't support a filter accurately, tell me before implementing a misleading version") rather than a misleading one — flagged here for visibility rather than treated as silently settled.

### 28.5 Privacy/security — verified against the actual current rules, not assumed
Read `firestore.rules`' real `users/{userId}` rule before writing any code that depends on it. **No rule changes were needed or made:**
- `allow read: if isSignedIn() && (request.auth.uid == userId || isAdmin())` — a member can only ever read their *own* doc; already exactly matches "not visible to other members." An unauthorized attempt to replicate the Owner Portal's `getDocs(collection(db,'users'))` query would be rejected by Firestore itself for a non-admin (a broad collection query can't satisfy an "own uid only" rule) — this is a real server-side guarantee, not just a UI-level gate.
- `allow create`/`allow update` have no field allowlist beyond protecting `role` — adding `state`/`city` to the write payload was already permitted, nothing needed loosening (and nothing was loosened).
- Location Analytics itself has no separate access path — it's rendered by the same client-side code as the rest of the People panel, behind the same `role === 'admin'` gate as everything else there.

### 28.6 Backward compatibility — existing members
An existing account with no `state`/`city` (any account created before this pass) continues to work exactly as before — sign-in, booking, everything, untouched. Their Member Profile's Location fields simply render blank (no forced re-registration, none was built). Location Analytics explicitly reports "Members With Location Data: X / Total" specifically so a blank-location member is never silently miscounted as "no location" being the same as "zero members" — they're counted in Total Members, just not in `byState`/`byCity`/regional breakdowns, which only ever include members who actually have a `state` set.

### 28.7 Demo data (`DEMO_MEMBERS`)
Extended from 3 bare entries to include Maya Johnson (previously only present via her booking/registration records, not as a standalone profile — added here with her own `state`/`city`) and ~19 new lightweight demo profiles spanning 10 states/regions with varied `createdAt` dates, purely so Location Analytics has enough realistic spread to actually demonstrate the state grid, Top Cities, regional breakdown, and All Time/Year/Month filters in preview mode. One entry (`demo-m23`) deliberately has no location at all, to demonstrate the "members without location data" case. One entry (`demo-m22`) deliberately uses a differently-cased city spelling ("macon" vs "Macon") to verify the normalization logic actually merges it rather than double-counting — checked by hand against the computation logic, not click-tested in a browser (see 28.8).

### 28.8 Verification performed (and its limits)
Same no-browser/no-emulator constraint as every prior phase. Checked: bracket-balance on `app.js` and brace-balance on `styles.css` after every edit (clean throughout); a `<div>`-balance check specifically on the new People-panel markup region; read `firestore.rules`' actual current text before relying on it (28.5), rather than assuming; traced `computeLocationAnalytics()`/`locFilterMembers()`/`normalizeCityKey()` by hand against the demo data in 28.7 to confirm the numbers they'd produce are correct (e.g., that "macon"/"Macon" merge into one Top Cities row, that the no-location member is excluded from `byState` but included in `totalMembers`). **Not verified:** actual rendered appearance of the tile-grid map, table layouts, or responsive behavior at any real viewport — this is the one thing most worth a close look during visual review, especially the tile-grid's approximate state positions (28.2).

### 28.9 Local preview
Running at **http://localhost:8080/** — sign in as the demo admin (`contactunveiledassembly@gmail.com`, any password on the preview site) → Owner Portal → People → Location Analytics tab. Nothing has been committed, pushed, or deployed.
